...
export async function POST(req: NextRequest, { params }: RouteContext) {
  try {
    ...
    await ShieldedExecutionService.delegate(
      agentId,
      owned.user.id,
      String(body.logic_account_pubkey ?? body.logicAccountPubkey ?? "").trim(),
      String(body.state_account_pubkey ?? body.stateAccountPubkey ?? "").trim(),
      String(body.permission_account_pubkey ?? body.permissionAccountPubkey ?? "").trim(),
      String(body.signed_tx ?? body.signedTx ?? "").trim()
    );
