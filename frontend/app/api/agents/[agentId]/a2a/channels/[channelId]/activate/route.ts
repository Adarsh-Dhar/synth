...
export async function POST(req: NextRequest, { params }: RouteContext) {
  try {
    ...
    await A2APaymentService.activateChannel(
      channelId,
      String(body.channel_account_pubkey ?? body.channelAccountPubkey ?? "").trim(),
      String(body.open_tx_signature ?? body.openTxSignature ?? "").trim()
    );
