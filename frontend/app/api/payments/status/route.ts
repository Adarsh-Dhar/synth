...
/**
 * Read credit balance directly from the DB column (added by the webhook handler).
 * Falls back to 0 if the column doesn't exist yet.
 */
async function getCreditBalanceFromDb(userId: string): Promise<number> {
  try {
    const result = await (prisma as any).$queryRawUnsafe(
      `SELECT "creditBalance" FROM "User" WHERE "id" = $1`,
      userId
    ) as Array<{ creditBalance: number | null }>;
    return Number(result?.[0]?.creditBalance ?? 0) || 0;
  } catch {
...
export async function GET(req: NextRequest) {
  ...
  // ── Resolve effective tier (use whichever is higher) ──────────────────────
  ...
  const [agentCount, runningCount, subscription, creditBalance] = await Promise.all([
    ...
    // Read from DB column directly — no external API calls
    getCreditBalanceFromDb(auth.user.id),
  ]);
