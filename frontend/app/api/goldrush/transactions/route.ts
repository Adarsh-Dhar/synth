...
export async function GET(req: NextRequest) {
  ...
  const url = `${GOLDRUSH_BASE_URL}/${network}/address/${encodeURIComponent(wallet)}/transactions_v3/`;
  ...
  if (!res.ok) {
    ...
    return NextResponse.json({ error: "Failed to fetch transactions.", detail: body.slice(0, 200) }, { status: 502 });
  }

  const json = await res.json();
  return NextResponse.json({ source: "goldrush", wallet, network, transactions: json }, { status: 200 });
}
