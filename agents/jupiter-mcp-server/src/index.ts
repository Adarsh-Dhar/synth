 ...
 *   build_swap_transaction  — Convert a quoteResponse into a signed-ready base64 transaction
...
if (!API_KEY) {
  process.stderr.write(
    "[jupiter-mcp] WARNING: JUPITER_API_KEY not set. Rate limits will apply to public endpoints.\n"
  );
}
...
const QUOTE_API = (
  process.env.JUPITER_QUOTE_API_URL ?? "https://quote-api.jup.ag/v6"
).replace(/\/+$/, "");
...
const BuildSwapSchema = z.object({
  quoteResponse: z.any().describe("The entire exact quoteResponse object returned from get_quote"),
  userPublicKey: z.string().min(32).max(44).describe("The wallet public key (base58) executing the trade"),
  wrapAndUnwrapSol: z.boolean().optional().default(true).describe("Auto wrap/unwrap SOL to wSOL"),
});
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (res.status === 429) {
        ...
        process.stderr.write(
          `[jupiter-mcp] 429 rate-limited — waiting ${wait}s (attempt ${attempt}/${MAX_RETRIES})\n`
        );
      ...
      if (res.status >= 500 && attempt < MAX_RETRIES) {
        process.stderr.write(
          `[jupiter-mcp] HTTP ${res.status} — retrying (attempt ${attempt}/${MAX_RETRIES})\n`
        );
...
async function handleSearchDocs(args: unknown) {
  ...
  if (scored.length === 0) {
    return {
      message:
        "No documentation found for query. Available topics: swap, trigger, tokens, lend, perps, price, copy_trade, safe_sniper.",
...
async function handleGetQuote(args: unknown) {
  const { input_mint, output_mint, amount, slippage_bps, swap_mode } =
    QuoteSchema.parse(args);
  ...
  return {
    quote,
    _note:
      "This is the quoteResponse object. Pass it directly to build_swap_transaction along with userPublicKey to get the base64 transaction.",
  };
}

// ── NEW: handleBuildSwapTransaction ──
async function handleBuildSwapTransaction(args: unknown) {
  const { quoteResponse, userPublicKey, wrapAndUnwrapSol } = BuildSwapSchema.parse(args);

  const url = `${QUOTE_API}/swap`;
  const body = JSON.stringify({
    quoteResponse,
    userPublicKey,
    wrapAndUnwrapSol,
    dynamicComputeUnitLimit: true, // Recommended by Jupiter for best execution
    prioritizationFeeLamports: "auto",
  });

  const swapData = await fetchJupiter(url, {
    method: "POST",
    body,
  });

  return {
    swapTransaction: (swapData as any).swapTransaction,
    _note: "This is the base64 encoded transaction. Deserialize it using VersionedTransaction, sign it with the user's Keypair, and send it to the RPC.",
  };
}
...
async function handleGetTokenInfo(args: unknown) {
  ...
  return {
    ...
    _safety_summary: {
      ...
      recommendation:
        token.strict && Number(token.daily_volume ?? 0) > 50_000
...
async function handleGenerateBotCode(args: unknown) {
  ...
  const strategyKeywordMap: Record<string, string> = {
  ...
  const templateDoc = DOCS.find(
    (d) => d.id === `bot_template_${strategy}` || d.keywords.includes(keyword)
  );
  ...
  return {
    ...
    // ── UPGRADED: auto-signs and sends via @solana/web3.js ──
...
// Provide WALLET_KEY as a JSON byte array [12,34,56...] in your .env
const secretKey = new Uint8Array(JSON.parse(process.env.WALLET_KEY || '[]'));
...
// ── Execution Engine ──
async function executeTransaction(base64Transaction: string) {
    try {
        const swapTransactionBuf = Buffer.from(base64Transaction, 'base64');
...
`,
    _instructions: [
      "1. Provide WALLET_KEY as a byte array and RPC_URL in your .env.",
      "2. Run `npm install @solana/web3.js node-fetch bs58`.",
      "3. Use the executeTransaction() function passing the base64 string from build_swap_transaction.",
    ],
  };
}
...
const TOOLS = [
  {
    name: "search_docs",
    description:
      "Search Jupiter API documentation. Use this FIRST before writing any Jupiter integration code. " +
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: "string",
          description:
            "Natural language query, e.g. 'how to place a limit order' or 'swap SOL to USDC'",
        },
        include_examples: {
          type: "boolean",
  ...
  },
  {
    name: "get_quote",
    description:
      "Fetch a live swap quote from Jupiter Quote API V6. " +
  ...
  },
  {
    name: "get_token_info",
    description:
      "Fetch token metadata and a safety assessment for an SPL token mint. " +
  ...
  },
  {
    name: "get_price",
    description:
      "Fetch current spot prices for one or more SPL tokens via Jupiter Price API V2. " +
  ...
  },
  {
    ...
    description:
      "Fetch all open trigger (limit/stop) orders for a wallet. " +
  ...
  },
  {
    ...
    description:
      "List all available Jupiter perpetual markets (SOL-PERP, BTC-PERP, etc.). " +
  ...
  // ── NEW: build_swap_transaction ──
  {
    name: "build_swap_transaction",
    description:
      "Takes a quoteResponse from get_quote and a userPublicKey, and returns the base64 encoded serialized transaction from Jupiter's /swap endpoint. " +
      "This is required to actually execute a trade.",
    inputSchema: {
      type: "object" as const,
      properties: {
        quoteResponse: { type: "object", description: "The exact quote response object" },
        userPublicKey: { type: "string", description: "Wallet public key (base58)" },
        wrapAndUnwrapSol: { type: "boolean", description: "Auto wrap/unwrap SOL (default true)" },
      },
  ...
  },
  {
    ...
    description:
      "Generate a complete, runnable TypeScript bot for a given trading strategy. " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        params: {
          type: "object",
          description:
            "Optional strategy parameters, e.g. { target_wallet: '7VHu...', position_size_usdc: 100 }",
        },
      },
      required: ["strategy"],
    },
  ...
  try {
    ...
    switch (name) {
      ...
      case "build_swap_transaction":
        result = await handleBuildSwapTransaction(rawArgs);
        break;
...
async function main() {
  ...
  process.stderr.write(
    `[jupiter-mcp] Server ready — ${TOOLS.length} tools loaded, ${DOCS.length} doc entries indexed.\n`
  );
}
