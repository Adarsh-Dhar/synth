...
if (!API_KEY) {
    process.stderr.write("[goldrush-mcp] FATAL: GOLDRUSH_API_KEY environment variable is missing.\n");
    process.exit(1);
}
const BASE_URL = (process.env.GOLDRUSH_BASE_URL ?? "https://api.covalenthq.com/v1").replace(/\/+$/, "");
...
// ─────────────────────────────────────────────
async function fetchGoldRush(endpoint, params = {}) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        ...
        try {
            ...
            if (res.status === 429) {
                ...
                process.stderr.write(`[goldrush-mcp] 429 rate-limited — waiting ${retryAfter}s (attempt ${attempt}/${MAX_RETRIES})\n`);
                await sleep(retryAfter * 1000);
                continue;
            }
            // Transient server errors
            if (res.status >= 500 && attempt < MAX_RETRIES) {
                process.stderr.write(`[goldrush-mcp] HTTP ${res.status} — retrying (attempt ${attempt}/${MAX_RETRIES})\n`);
...
const TOOLS = [
    {
        ...
        description: "Fetch current SPL token balances with exact fiat USD values for a Solana wallet. " +
    ...
    },
    {
        ...
        description: "Fetch decoded transaction history (swaps, transfers, staking) for a Solana wallet. " +
    ...
    },
    {
        ...
        description: "Fetch the top holders of an SPL token mint. " +
    ...
    },
    {
        ...
        description: "Fetch the historical USD portfolio value for a Solana wallet over N days. " +
    ...
    },
    {
        ...
        description: "Fetch spot price and OHLCV candle data for an SPL token. " +
...
// Tool handlers
// ─────────────────────────────────────────────
async function handleGetTokenBalances(args) {
    const { wallet } = WalletSchema.parse(args);
    const data = await fetchGoldRush(`/${CHAIN_ID}/address/${encodeURIComponent(wallet)}/balances_v2/`);
    return data;
}
async function handleGetTransactions(args) {
    const { wallet, page, page_size } = TransactionsSchema.parse(args);
    const data = await fetchGoldRush(`/${CHAIN_ID}/address/${encodeURIComponent(wallet)}/transactions_v3/`, { "page-number": page, "page-size": page_size });
    return data;
}
async function handleGetTokenHolders(args) {
    const { token_address, page_size } = HoldersSchema.parse(args);
    const data = await fetchGoldRush(`/${CHAIN_ID}/tokens/${encodeURIComponent(token_address)}/token_holders_v2/`, { "page-size": page_size });
    return data;
}
async function handleGetHistoricalPortfolio(args) {
    const { wallet, days } = HistoricalSchema.parse(args);
    const data = await fetchGoldRush(`/${CHAIN_ID}/address/${encodeURIComponent(wallet)}/portfolio_v2/`, { days });
    return data;
}
async function handleGetTokenPrice(args) {
    ...
    if (to)
        params["to"] = to;
    const data = await fetchGoldRush(`/pricing/historical_by_addresses_v2/${CHAIN_ID}/USD/${encodeURIComponent(token_address)}/`, params);
    return data;
}
// ─────────────────────────────────────────────
// MCP Server bootstrap
// ─────────────────────────────────────────────
const server = new Server({ name: "goldrush-solana-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (err instanceof z.ZodError) {
            throw new McpError(ErrorCode.InvalidParams, `Invalid arguments for tool "${name}": ${err.errors
...
async function main() {
    ...
    process.stderr.write(`[goldrush-mcp] Server ready — chain: ${CHAIN_ID}, base: ${BASE_URL}\n`);
}
main().catch((err) => {
